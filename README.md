...
```julia
    features = extract_radiomic_features(ct.raw, mask.raw, spacing; features=[:glcm, :glszm], bin_width=25.0f0, verbose=true);
```
To compute with a specific number of bins
```julia
    features = extract_radiomic_features(ct.raw, mask.raw, spacing; features=[:glcm, :glszm], n_bins=16, verbose=true);
```
