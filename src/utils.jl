...
"""
    apply_scaling(nifti_image)

Applies scaling (slope/intercept) to a NIFTI image if necessary.
Returns an array with values in Hounsfield units (HU).

# Arguments:
- `nifti_image`: NIFTI image whose header contains `scl_slope` and `scl_inter`

# Returns:
- Array with scaled values (or the original values if scaling is not needed)
"""
function apply_scaling(nifti_image)
    slope = nifti_image.header.scl_slope
    intercept = nifti_image.header.scl_inter
    
    if slope != 1.0 || intercept != 0.0
        println("Applying scaling: slope=$slope, intercept=$intercept")
        return nifti_image.raw .* slope .+ intercept
    else
        println("No scaling needed (slope=$slope, intercept=$intercept)")
        return nifti_image.raw
    end
end
"""
    process_input_data(img_input, mask_input, voxel_spacing_input, verbose)

Automatically processes input data, handling both NIFTI objects and arrays.
If `img_input` is a NIFTI object, it automatically applies scaling and extracts the voxel spacing.

# Arguments:
- `img_input`: NIFTI object or array
- `mask_input`: NIFTI object or array
- `voxel_spacing_input`: Voxel spacing (optional if `img_input` is a NIFTI object)
- `verbose`: If True, prints informational messages

# Returns:
- `img_array`: Processed image array (with scaling if NIFTI)
- `mask_array`: Mask array
- `voxel_spacing`: Voxel spacing
"""
function process_input_data(img_input, mask_input, voxel_spacing_input, verbose::Bool)
    if hasproperty(img_input, :header) && hasproperty(img_input, :raw)
        if verbose
            println("NIFTI image detected, applying automatic scaling...")
        end
        img_array = apply_scaling(img_input)
        
        if isnothing(voxel_spacing_input)
            voxel_spacing_input = [img_input.header.pixdim[2], 
                                   img_input.header.pixdim[3], 
                                   img_input.header.pixdim[4]]
            if verbose
                println("Extracted voxel spacing from NIFTI: $voxel_spacing_input")
            end
        end
    else
        img_array = img_input
    ...
    if hasproperty(mask_input, :raw)
        mask_array = mask_input.raw
    else
        mask_array = mask_input
    end
    
    if isnothing(voxel_spacing_input)
        throw(ArgumentError("voxel_spacing must be provided when img_input is not a NIFTI object"))
    end
    
    return img_array, mask_array, voxel_spacing_input
end
